...
void vcRender_RenderScene(vcState *pProgramState, vcRenderContext *pRenderContext, vcRenderData &renderData, vcFramebuffer *pDefaultFramebuffer)
{
  ...
  if (pProgramState->settings.activeViewportCount > 1 && !pProgramState->settings.camera.mapMode[0] && pProgramState->settings.camera.mapMode[pProgramState->activeViewportIndex] && pProgramState->settings.presentation.showCameraFrustumInMapMode)
